...
# -----------------------------------------------------------------------------
# STREAMLIT PAGE CONFIG & MOBILE OPTIMIZED CSS
# -----------------------------------------------------------------------------
st.set_page_config(
...
# Mobile-Optimized CSS to prevent flickering, blank screens, and table clipping
st.markdown("""
<style>
...
# -----------------------------------------------------------------------------
# 4. DASHBOARD RENDERER & BALANCED RESPONSIVE MOBILE UI (FIXED RENDER ENGINE)
# -----------------------------------------------------------------------------
def build_html_view(rows, timestamp, latency_ms, total_scanned, is_locked=False):
    ...
    if not rows:
        ...
        for r in rows:
            ...
            imb_color = '#00e676' if r['imbalance'] >= 0 else '#ff5252'
            
            row_html = (
                f"<tr style='border-bottom: 1px dashed #30363d;'>"
                f"<td style='font-weight: 900; text-align: left; color: #ffffff; padding: 10px 8px; border-right: 1px dashed #21262d; white-space: nowrap;'>{r['symbol']}</td>"
                f"<td style='text-align: left; font-size: 9.5px; line-height: 1.35; border-right: 1px dashed #21262d; padding: 6px 8px; white-space: nowrap;'>{r['zone_html']}</td>"
                f"<td style='border-right: 1px dashed #21262d; white-space: nowrap;'>Rs {r['open']:.2f}</td>"
                f"<td style='font-weight: 700; border-right: 1px dashed #21262d; white-space: nowrap;'>Rs {r['ltp']:.2f}</td>"
                f"<td style='color: {pnl_color}; font-weight: 800; border-right: 1px dashed #21262d; white-space: nowrap;'>{r['pnl']:+.2f}%</td>"
                f"<td style='border-right: 1px dashed #21262d;'>{emas_html}</td>"
                f"<td style='padding: 5px 6px; border-right: 1px dashed #21262d;'>{r['pressure_box']}</td>"
                f"<td style='padding: 5px 6px; border-right: 1px dashed #21262d;'>{r['cri_box']}</td>"
                f"<td style='padding: 5px 6px; border-right: 1px dashed #21262d;'>{r['sl_box']}</td>"
                f"<td style='color: #00e676; font-weight: 800; border-right: 1px dashed #21262d; white-space: nowrap;'>Rs {r['target']:.2f}</td>"
                f"<td style='border-right: 1px dashed #21262d; white-space: nowrap;'><span style='color: #00e676; font-weight: 900; font-size: 12px;'>{r['tcs']}/100</span></td>"
                f"<td style='color: {imb_color}; font-weight: 700; white-space: nowrap; padding: 6px 10px;'>{r['cobi_html']}</td>"
                f"</tr>"
            )
    ...
    full_html = (
        f"<div class='mobile-table-container'>"
        f"<div style='display: flex; flex-direction: column; gap: 6px; border-bottom: 1px dashed #30363d; padding-bottom: 8px; margin-bottom: 10px;'>"
        f"<div style='color: #00e676; font-size: 13px; font-weight: 900; letter-spacing: 0.5px;'>⚡ FALCON QUANT ENGINE | SMC LIVE ALPHA TERMINAL</div>"
        f"<div style='color: #8b949e; font-size: 10.5px; background: #161b22; padding: 4px 8px; border-radius: 4px; border: 1px dashed #30363d; word-wrap: break-word;'>"
        f"{lock_badge} <br> LIVE: {timestamp} IST | Active: {len(rows)}/{total_scanned} | Latency: {latency_ms}ms"
        f"</div>"
        f"</div>"
        f"<table style='width: 100%; border-collapse: collapse; font-size: 11px; text-align: center; border: 1px dashed #30363d; font-family: -apple-system, BlinkMacSystemFont, \"Segoe UI\", monospace; color: #c9d1d9;'>"
        f"<thead>"
        f"<tr style='background: #161b22; color: #8b949e; text-transform: uppercase; font-size: 9.5px; border-bottom: 1px dashed #30363d;'>"
        f"<th style='text-align: left; padding: 8px 6px; border-right: 1px dashed #30363d;'>Symbol</th>"
        f"<th style='text-align: left; padding: 8px 6px; border-right: 1px dashed #30363d;'>Zone Alignments</th>"
        f"<th style='border-right: 1px dashed #30363d;'>Open</th>"
        f"<th style='border-right: 1px dashed #30363d;'>LTP</th>"
        f"<th style='border-right: 1px dashed #30363d;'>Change</th>"
        f"<th style='border-right: 1px dashed #30363d;'>EMAS (1m|3m|5m|15m)</th>"
        f"<th style='border-right: 1px dashed #30363d; min-width: 140px;'>Supply/Demand Delta Box</th>"
        f"<th style='border-right: 1px dashed #30363d; min-width: 140px;'>Reversal Engine (CRI)</th>"
        f"<th style='border-right: 1px dashed #30363d; min-width: 115px;'>SL / Best Entry</th>"
        f"<th style='border-right: 1px dashed #30363d;'>Target</th>"
        f"<th style='border-right: 1px dashed #30363d;'>TCS Score</th>"
        f"<th style='padding: 8px 10px; min-width: 125px;'>Buyer/Seller (COBI)</th>"
        f"</tr>"
        f"</thead>"
        f"<tbody>"
        f"{rows_str}"
        ...
        try:
            ...
            for ticker in tickers_to_query:
                ...
                try:
                    ...
                    sl_box_html = (
                        f"<div style='border: 1px dashed {sl_border_color}; background-color: {sl_bg_color}; padding: 3px 5px; border-radius: 4px; text-align: center;'>"
                        f"<div style='font-size: 8.5px; font-weight: 800; color: {sl_border_color}; white-space: nowrap;'>{sl_title}</div>"
                        f"<div style='font-size: 11px; font-weight: 900; color: #ffffff;'>Rs {sl_best_entry:.2f}</div>"
                        f"</div>"
                    )
                    ...
                    pressure_box_html = (
                        ...
                        f"<div style='font-size: 8.5px; font-weight: 800; color: {border_c}; white-space: nowrap;'>{status_t}</div>"
                    ...
                    cri_box_html = (
                        f"<div style='border: 1px dashed {cri_border}; background-color: {cri_bg}; padding: 3px 5px; border-radius: 4px; text-align: center;'>"
                        f"<div style='font-size: 8.5px; font-weight: 800; color: {action_color}; white-space: nowrap;'>{cri_status}</div>"
                        f"<div style='font-size: 11px; font-weight: 900; color: #ffffff;'>{cri_val:.1f}%</div>"
                        f"<div style='color: {action_color}; font-size: 8px; font-weight: 900; margin-top: 1px; white-space: nowrap;'>⚡ {cri_action}</div>"
                        f"</div>"
                    )
            ...
            placeholder.markdown(
                build_html_view(display_rows, now_time, elapsed_ms, len(STREAM_TICKERS), st.session_state.is_universe_locked),
                unsafe_allow_html=True
            )
