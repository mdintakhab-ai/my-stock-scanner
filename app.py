...
def build_html_view(rows, timestamp, latency_ms, total_scanned, is_locked=False):
    rows_str = ""
    lock_badge = "<span style='color:#00e676; font-weight:bold;'>ðŸ”’ UNIVERSE LOCKED (ALPHA FREEZE)</span>" if is_locked else "<span style='color:#ffaa00; font-weight:bold;'>âš¡ STREAMING QUANT DESK...</span>"
    ...
    if not rows:
        ...
        for r in rows:
            def ema_dot(b):
                if b:
                    return "<span style=\"display:inline-block;width:7px;height:7px;border-radius:50%;background:#00e676;vertical-align:middle;margin-right:2px;\"></span>"
                return "<span style=\"display:inline-block;width:7px;height:7px;border-radius:50%;background:#ff5252;vertical-align:middle;margin-right:2px;\"></span>"
            emas_html = (
                "<div style='display:flex;gap:7px;align-items:center;justify-content:center;white-space:nowrap;font-size:8px;font-weight:800;'>"
                f"<span>1m {ema_dot(r['e1'])}</span>"
                f"<span>3m {ema_dot(r['e3'])}</span>"
                f"<span>5m {ema_dot(r['e5'])}</span>"
                f"<span>15m {ema_dot(r['e15'])}</span>"
                "</div>"
            )
    ...
    return f"""
    <div class="falcon-mobile-wrap" style="background-color:#090c10; border:1.5px solid #30363d; border-radius:8px; padding:10px; font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',monospace; color:#c9d1d9; width:100%; box-sizing:border-box;">
        <style>
            .falcon-scroll {{
                width:100%;
                overflow-x:auto;
        ...
        <div class="falcon-scroll">
        <div class="falcon-head" style="display:flex; justify-content:space-between; align-items:center; border-bottom:1px dashed #30363d; padding-bottom:8px; margin-bottom:10px;">
            <div style="color: #00e676; font-size: 13px; font-weight: 900; letter-spacing: 0.5px;">âš¡ FALCON QUANT ENGINE | SMC LIVE ALPHA TERMINAL</div>
...
# -----------------------------------------------------------------------------
# -----------------------------------------------------------------------------
# 5. STREAMLIT MOBILE EXECUTION ENGINE
# -----------------------------------------------------------------------------
st.set_page_config(
    page_title="Falcon Quant Engine",
    page_icon="âš¡",
    layout="wide",
...
def _fetch_one(ticker):
    return ticker, fetch_ticker_data_instant(ticker)


# Persistent front-end component: unlike components.html(), this iframe stays
# mounted while fragment data changes. JavaScript receives new HTML through
# Streamlit's render event and patches the existing DOM instead of recreating
# the whole iframe on every refresh.
_COMPONENT_DIR = Path(__file__).resolve().parent / "falcon_live_component"
_falcon_live_component = components.declare_component(
    "falcon_live_terminal",
    path=str(_COMPONENT_DIR),
)


@st.fragment(run_every=2.0)
def run_live_streamlit():
    t0 = time.time()

    # Session state is used as a short-lived live cache. A transient Yahoo
    # failure therefore never blanks the dashboard or removes a locked row.
    if "last_good_rows" not in st.session_state:
        st.session_state.last_good_rows = {}
    ...
    # Parallel I/O is the main latency fix. The original sequential 30-ticker
    # loop waits for every HTTP request one after another. Here the same
    # per-ticker calculation is preserved, but network waits overlap.
    with ThreadPoolExecutor(max_workers=8) as executor:
        futures = [executor.submit(_fetch_one, ticker) for ticker in STREAM_TICKERS]
        fetched = {}
    ...
    for ticker in STREAM_TICKERS:
        ...
        if df is None or df.empty or len(df) < 3:
            # Preserve the last successful row for a locked symbol.
            cached = st.session_state.last_good_rows.get(ticker)
            if cached is not None:
                current_rows.append(cached)
            continue
        ...
        try:
            ...
            retest_html = "<div style='color:#ffaa00; font-size:9px; font-weight:bold; margin-top:2px; white-space:nowrap;'>âš ï¸ ZONE RE-TEST REJECTION</div>" if pressure_pct > 75.0 else ""
            ...
            if cri_val >= 80.0:
                ...
                cri_border, cri_bg, action_color = "#ffaa00", "rgba(255,170,0,0.12)", "#ffaa00"
            elif cri_val >= 40.0:
                cri_border, cri_bg, action_color = "#ffc107", "rgba(255,193,7,0.08)", "#ffc107"
            else:
                cri_border, cri_bg, action_color = "#30363d", "#161b22", "#8b949e"
            ...
            <div style='border:1px dashed {cri_border}; background-color:{cri_bg}; padding:3px 5px; border-radius:4px; text-align:center;'>
                ...
                <div style='color:{action_color}; font-size:8px; font-weight:900; margin-top:1px; white-space:nowrap;'>âš¡ {cri_action}</div>
            </div>
            """
            ...
            if tcs >= HIGH_CONVICTION_TCS_THRESHOLD:
                row = {
                    "ticker": ticker, "symbol": ticker.replace(".NS", ""), "zone_html": zone_html,
                    "open": open_p, "ltp": ltp, "pnl": pnl_pct, "pressure_box": pressure_box_html,
                    "cri_box": cri_box_html, "sl_box": sl_box_html, "target": target_price,
                    "tcs": tcs, "cobi_html": cobi_html, "imbalance": imbalance_pct,
                    "e1": ltp > ema1, "e3": ltp > ema3, "e5": ltp > ema5, "e15": ltp > ema15
                }
                st.session_state.last_good_rows[ticker] = row
    ...
    dashboard_eligible_rows = [
        ...
        if DASHBOARD_MIN_PRICE <= r['ltp'] <= DASHBOARD_MAX_PRICE
    ]
    ...
    if st.session_state.is_universe_locked and st.session_state.locked_symbols:
        display_rows = [
            st.session_state.last_good_rows.get(t, r)
            for t in st.session_state.locked_symbols
            for r in dashboard_eligible_rows
            if r['ticker'] == t
        ]
        # If a locked ticker is temporarily absent from the fresh filter, keep
        # its previous row instead of blinking/removing it.
        for ticker in st.session_state.locked_symbols:
            if not any(r['ticker'] == ticker for r in display_rows):
                cached = st.session_state.last_good_rows.get(ticker)
                if cached is not None:
                    display_rows.append(cached)
        display_rows.sort(key=lambda x: st.session_state.locked_symbols.index(x['ticker']))
    else:
    ...
    # IMPORTANT: use the legacy component renderer here instead of st.html().
    # It renders the HTML table as an actual DOM inside the component and avoids
    # the raw <tr>/<td> text problem seen on the deployed app.
    table_html = build_html_view(
        display_rows, now_time, elapsed_ms, len(STREAM_TICKERS),
        st.session_state.is_universe_locked
    )
    component_height = max(230, min(720, 165 + (len(display_rows) * 92)))
    _falcon_live_component(
        html=table_html,
        height=component_height,
        key="falcon-live-terminal",
        default=None,
    )
